		...
		if($this->is_login){
			...
			if(strlen($search_name_valid_str) > 0){
				...
				if(strlen($search_name_valid_str) > 0){
					...
					if(xunService()){	                                         
                                                require_once LIB_PATH.'xunsearch/php/lib/XS.php';
                                                $xs=new \XS('contacts_base_info');
                                                $res=$xs->search->setLimit($page_size,($page_no-1)*$page_size)->search('user_id:'.$this->user_id.' is_delete:'.$is_recy.' '.$search_name_valid_str);
                                                if($res){
                                                    foreach($res as $key=>$val){
                                                        $result[$key]['id']=$val->id;
                                                        $result[$key]['user_id']=$val->user_id;
                                                        $result[$key]['is_delete']=$val->is_delete;
                                                        $id_array[]=$val['id'];
                                                    }
                                                }
						if(!empty($result)){
							$return_json['total_count'] = count($result);
