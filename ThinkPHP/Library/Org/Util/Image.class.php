...
class Image {
     ...
     * @return string
     */
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify',$tty_file = '',$file_size='') {
        ...
        $check_imagettftext = function_exists('imagettftext')&& file_exists($tty_file) ? true : false;       
        for ($i = 0; $i < $length; $i++) {
        	
        	if($check_imagettftext){
        		
        		imagettftext($im,$file_size,0, $i * $file_size + 5, mt_rand(24, 32), $stringColor, $tty_file, $randval{$i});
        	}else{
